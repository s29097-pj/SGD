...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, L_SQUIR_IMG, R_SQUIR_IMG, GRASSIMAGES, HEART_IMG, HEART_EMPTY_IMG
    ...
    HEART_IMG = pygame.transform.scale(pygame.image.load('heart.png'), (40, 40)) # heart image
    HEART_EMPTY_IMG = pygame.transform.scale(pygame.image.load('heart_empty.png'), (40, 40)) # empty heart image
    GRASSIMAGES = []
    for i in range(1, 5):
...
def runGame():
    ...
    level = 1             # the current level of the game
    currentFps = FPS # the current frames per second
    levelUpTime = 0 # time the player leveled up
    levelUpDisplayTime = 1.5 # time the player leveled up
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        if score % 5 == 0:
                            level += 1
                            levelUpTime = time.time() + levelUpDisplayTime
                            # increase the number of squirrels in the game
                            if len(squirrelObjs) < NUMSQUIRRELS + level * 2:
                                squirrelObjs.append(makeNewSquirrel(camerax, cameray))
                            # increase FPS, if the player is level 5 or higher
                            currentFps = min(currentFps + 2, 60)
...
# Heart - drawHealthMeter:
def drawHealthMeter(currentHealth):
    for i in range(MAXHEALTH):
        x = 10 + i * (HEART_IMG.get_width() + 5) # 10 px gap
        y = 50  # below the score
        if i < currentHealth:
            DISPLAYSURF.blit(HEART_IMG, (x, y)) # draw a filled heart
        else:
            DISPLAYSURF.blit(HEART_EMPTY_IMG, (x, y)) # draw an empty heart
