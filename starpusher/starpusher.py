...
import random, sys, copy, os, pygame, time # Add time
...
def main():
    ...
    IMAGESDICT = {'uncovered goal': pygame.image.load('RedSelector.png'),
                  ...
                  'level_complete': pygame.image.load('level_complete.png'),
...
def runLevel(levels, levelNum):
    ...
    levelIsComplete = False # set to True when the level is solved
    levelIsCompleteTime = None # new variable to track time
    ...
    while True: # main game loop
        ...
        if levelIsComplete:
            if levelIsCompleteTime is None:
                levelIsCompleteTime = time.time() # Set the time when the level was completed
            # Show the "Level Complete!" image
            completeRect = IMAGESDICT['level_complete'].get_rect(center=(HALF_WINWIDTH, HALF_WINHEIGHT))
            DISPLAYSURF.blit(IMAGESDICT['level_complete'], completeRect)
            if time.time() - levelIsCompleteTime >= 3: # Wait for 3 seconds
                return 'solved'
